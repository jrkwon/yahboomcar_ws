...
class msgToimg:
    def __init__(self):
        ...
        self.cam_image_sub = rospy.Subscriber("Camera/image_msg", Image_Msg, self.cam_image_sub_callback)
        self.pub_img = rospy.Publisher("yoloDetect/image", Image, queue_size=100)
        self.pub_cam_img = rospy.Publisher("/usb_cam", Image,queue_size=1)
    ...
    def cam_image_sub_callback(self,data):
        if not isinstance(data, Image_Msg): return
        # 将自定义图像消息转化为图像
        # Convert custom image messages to images
        image = np.ndarray(shape=(data.height, data.width, data.channels), dtype=np.uint8,
