...
class Traffic_Identify:
        ...
        if pred != [None]:
            for i, det in enumerate(pred):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    print("mid_x:",mid_x)
                    print("mid_y:",mid_y)
                    '''print(x1y1x2y2[0][0].item())#左上角方框x
                    print(x1y1x2y2[0][1].item())#右上角方框y
                    print(x1y1x2y2[0][2].item())#右下角方框x
                    print(x1y1x2y2[0][3].item())#右下角方框y'''
                    #plot_one_box(xyxy, frame, label=label, color=colors[int(cls)], line_thickness=3)
                    #print(conf)
                    if( mid_y> 214 and mid_y<223):
                        plot_one_box(xyxy, frame, label=label, color=colors[int(cls)], line_thickness=3)
                        if label == "Turn_right":
                            a = threading.Thread(target=self.execute, args=(mid_x,)).start()
                            print(a)
                            #print("z_Pid",z_Pid)
    ...
    def execute(self,x_set):
        ...
        twist = Twist()
        [z_Pid, _] = self.PID_controller.update([(x_set - 320)*1.0/8, 0]) 
        twist.angular.z = -z_Pid
        twist.linear.x = 0.5
        self.ros_ctrl.pub_cmdVel.publish(twist)
