...
void ImageConverter::imageCb(const sensor_msgs::ImageConstPtr &msg) {
    ...
    if (action == 'q' || action == ACTION_ESC) this->Cancel(),printf("exit!\n");
    else if (action == 'r')  this->Reset(),printf("reset!\n");
    else if (action == ACTION_SPACE) enable_get_depth = true,printf("space!\n");
}
...
void ImageConverter::depthCb(const sensor_msgs::ImageConstPtr &msg) {
    ...
    if (enable_get_depth) {
        ...
        ROS_WARN("distance = %.3f,center_x =  %d", distance, center_x);
    }
